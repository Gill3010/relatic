...
const JournalMetrics = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://relaticpanama.org/_events/api/journalsMetrics.php')
      .then(res => res.json())
      .then(data => {
  ...
  const cards = [
    { label: 'Revistas', value: metrics.journals },
    { label: 'Artículos', value: metrics.articles },
  ...
  const pieData = [
    { name: 'Revistas', value: metrics.journals },
    { name: 'Artículos', value: metrics.articles },
  ...
  return (
    ...
    >
      <h2 className="text-xl md:text-2xl font-bold text-center mb-4 text-[#0a2d4d]">
        📊 Métricas de Revistas Indexadas
      </h2>
      ...
      >
        ...
        {/* Leyenda minimalista */}
        <div className="flex justify-center gap-3 mt-2 text-xs text-gray-700">
